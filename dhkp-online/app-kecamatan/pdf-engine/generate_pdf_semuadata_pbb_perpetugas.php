...
if (isset($_SESSION['desaXweb'])) {
	...
	try {
		...
		if ($stmt_cnt->execute()) {
			...
			if (intval($rows)>0) {
				$stmt_sum = $dbcon->prepare("SELECT SUM(PBB_YG_HARUS_DIBAYAR_SPPT) AS sumtotal FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX = :idxpetugas");
				...
				$pdf->Cell(9.75,0.5,'SEMUA DATA','TR',1,'L','');
