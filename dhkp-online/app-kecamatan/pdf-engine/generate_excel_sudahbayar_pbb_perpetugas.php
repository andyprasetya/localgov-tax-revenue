...
if (isset($_SESSION['desaXweb'])) {
	...
	try {
		...
		/* counting data */
		$stmt_cnt = $dbcon->prepare("SELECT COUNT(*) as foundrows FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = :idxpetugas");
		...
		if ($stmt_cnt->execute()) {
			...
			if (intval($rowsfound)>0) {
				/* SUM total */
				$stmt_sum_total = $dbcon->prepare("SELECT SUM(PBB_YG_HARUS_DIBAYAR_SPPT) as totaltargetpetugas FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = :idxpetugas");
				...
				/* SUM assigned */
				$stmt_sum_assigned = $dbcon->prepare("SELECT SUM(JML_SPPT_YG_DIBAYAR) as totalassigneddibayar FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = :idxpetugas");
				...
				/* select all assigned */
				$stmt = $dbcon->prepare("SELECT * FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = :idxpetugas");
				...
				if ($stmt->execute()) {
					...
					$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue('B2', 'STATUS: SUDAH BAYAR')
						...
						->setCellValue('H8', 'TANGGAL BAYAR')
						->setCellValue('I8', 'JUMLAH PEMBAYARAN');
					while($rowset = $stmt->fetch(PDO::FETCH_ASSOC)){
						...
						$objPHPExcel->setActiveSheetIndex(0)
									...
									->setCellValue('H'.$rowcount.'', ''.strval(substr($rowset['TGL_PEMBAYARAN_SPPT'],8,2).'/'.substr($rowset['TGL_PEMBAYARAN_SPPT'],5,2).'/'.substr($rowset['TGL_PEMBAYARAN_SPPT'],0,4)).'')
									->setCellValue('I'.$rowcount.'', ''.number_format($rowset['JML_SPPT_YG_DIBAYAR'],0,'','').'');
						/* formatting */
						...
						$objPHPExcel->getActiveSheet()
									...
									->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
						$objPHPExcel->getActiveSheet()
									->getStyle('I'.$rowcount.'')->getNumberFormat()
									->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
						$rowcount++;
						$rownum++;
					}
...
header('Content-Disposition: attachment;filename="REKAPITULASI_PER_PETUGAS_'.trim($namaPetugas).'_SUDAH_BAYAR_'.Date('dmY_His').'.xlsx"');
