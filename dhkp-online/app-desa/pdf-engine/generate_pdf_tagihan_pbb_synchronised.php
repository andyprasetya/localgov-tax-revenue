...
if (isset($_SESSION['desaXweb'])) {
	...
	try {
		...
		if ($stmt_cnt->execute()) {
			...
			$stmt_sum_unpaid_sppt = $dbcon->prepare("SELECT SUM(PBB_YG_HARUS_DIBAYAR_SPPT) as totalsumunpaidspptdesa FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND STATUS_PEMBAYARAN_SPPT = 0 AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01'");
			$stmt_sum_unpaid_sppt->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
			$stmt_sum_unpaid_sppt->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
			$stmt_sum_unpaid_sppt->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
			$stmt_sum_unpaid_sppt->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR);
			$stmt_sum_unpaid_sppt->bindValue(":thnpajak", $thnsppt, PDO::PARAM_STR);
			if ($stmt_sum_unpaid_sppt->execute()) {
				while($rowset_sum_unpaid_sppt = $stmt_sum_unpaid_sppt->fetch(PDO::FETCH_ASSOC)){
					$sumresult = $rowset_sum_unpaid_sppt['totalsumunpaidspptdesa'];
				}
				/* select all sppt */
				...
				if ($stmt_select_data->execute()) {
					...
					$pagenum = 1;
					$pdf->SetFont('Arial','',8);
					$pdf->Cell(19.5,0.5,'DAFTAR TAGIHAN','',1,'C','');
...
/* ##################################################################################### */
						...
						if (intval($rows) <= $maxonepagerows) { /* rows <= 32 */
							...
							if (intval($rownum) == 1) {
								...
								$pdf->Cell(3,0.5,number_format($rowset['PBB_YG_HARUS_DIBAYAR_SPPT'],0,',','.'),'R',1,'R','');
								...
								$pdf->Cell(17.5,0.75,'Total Tagihan:','LRBT',0,'R','');
							...
							if (intval($rownum) == 1) {
								...
								$pdf->Cell(2,0.5,'hal 1'.number_format($subtotal,2,',','.'),'RBT',1,'R','');
									/* total sum row - last row */
								$pdf->Cell(17.5,0.75,'Total Tagihan:','LRBT',0,'R','');
								$pdf->Cell(2,0.75,number_format($sumresult,2,',','.'),'RBT',1,'R','');
								$pagenum++;
								...
								$subtotal += $rowset['PBB_YG_HARUS_DIBAYAR_SPPT'];
								/* total sum row - last row */
								$pdf->Cell(17.5,0.5,'Subtotal:','LRBT',0,'R','');
								$pdf->Cell(2,0.5,number_format($subtotal,0,',','.'),'RBT',1,'R','');
									/* total sum row - last row */
								$pdf->Cell(17.5,0.75,'Total Tagihan:','LRBT',0,'R','');
								$pdf->Cell(2,0.75,number_format($sumresult,2,',','.'),'RBT',1,'R','');
								$pagenum++;
