...
$objPHPExcel->getProperties()->setCreator("MAPATDA BKAD")
							 ...
							 ->setTitle("DAFTAR HIMPUNAN KETETAPAN PAJAK")
							 ->setSubject("PAJAK BUMI DAN BANGUNAN PERDESAAN DAN PERKOTAAN ( PBB-P2 )")
							 ->setDescription("DHKP Desa")
							 ->setKeywords("BKAD,DHKP,Desa")
				...
				if (!$dbcon) {
					...
					if (oci_execute($stmt)) {
						...
						$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('B1', 'DAFTAR HIMPUNAN KETETAPAN PAJAK (DHKP)')
							...
							->setCellValue('B6', 'TAHUN PAJAK: '.$appxinfo['_tahun_pajak_'].'')
...
header('Content-Disposition: attachment;filename="DHKP_'.$kode_propinsi.''.$kode_dati2.''.$kode_kecamatan.''.$kode_kelurahan.'_'.$namaKelurahan.'_'.Date('dmY_His').'.xlsx"');
