	...
	try {
		...
		if ($stmt_cnt->execute()) {
			...
			if ($stmt_sum_lembar_sppt->execute()) {
				...
				/* select all sppt */
				/* $stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak ORDER BY KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP"); */
				...
				if ($stmt_select_data->execute()) {
					...
					$pdf->Cell(19.5,4,'BKAD  KABUPATEN  KULON PROGO','',1,'C','');
...
/* ##################################################################################### */
						...
						if (intval($rows) <= $maxonepagerows) { /* rows <= 32 */
							...
							if (intval($rownum) == 1) {
								...
								$pdf->Cell(9.75,0.5,'','',0,'C','');
								$pdf->Cell(9.75,0.5,'KEPALA BADAN KEUANGAN DAN ASET DAERAH,','',1,'C','');
