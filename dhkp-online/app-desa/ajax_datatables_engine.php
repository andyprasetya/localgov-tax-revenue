...
if (isset($_GET['cmdx'])){
	...
	if ($_GET['cmdx'] == "GET_PETUGAS_PENAGIHAN") {
			...
			try {
				...
				$stmt = $dbcon->prepare("SELECT * FROM appx_desa_petugas_pungut WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa");
	...
	/*----------------------------------------------------------------
	 * RETRIEVE DATA MATERI DISTRIBUSI SPPT/NOP KE PETUGAS PEMUNGUT BY TAX YEAR
	 * ---------------------------------------------------------------*/
    } elseif ($_GET['cmdx'] == "GET_MATERI_DISTRIBUSI_SPPT_BYTAXYEAR") {
			...
			try {
				...
				$stmt_cnt->bindValue(":kdkec", $_SESSION['desaXweb']['KD_KECAMATAN'], PDO::PARAM_STR);
				$stmt_cnt->bindValue(":kddesa", $_SESSION['desaXweb']['KD_KELURAHAN'], PDO::PARAM_STR);
				$stmt_cnt->bindValue(":thnpajak", $taxyear, PDO::PARAM_STR);
				if ($stmt_cnt->execute()) {
					while($rowset_cnt = $stmt_cnt->fetch(PDO::FETCH_ASSOC)){
						$rowsfound = $rowset_cnt['foundrows'];
					}
					if (intval($rowsfound)>0) {
						...
						$stmt->bindValue(":kdkec", $_SESSION['desaXweb']['KD_KECAMATAN'], PDO::PARAM_STR);
						$stmt->bindValue(":kddesa", $_SESSION['desaXweb']['KD_KELURAHAN'], PDO::PARAM_STR);
						$stmt->bindValue(":thnpajak", $taxyear, PDO::PARAM_STR);
						$stmt->execute();
						while($rowset = $stmt->fetch(PDO::FETCH_ASSOC)){
							$items[] = $rowset;
						}
	...
	/*----------------------------------------------------------------
	 * RETRIEVE DATA SPPT/NOP YANG AKAN DIPINDAHKAN PETUGAS PEMUNGUTNYA
	 * ---------------------------------------------------------------*/
    } elseif ($_GET['cmdx'] == "GET_MATERI_DISTRIBUSI_SPPT_EDIT") {
			try {
				$dbcon = new PDO("mysql:host=".$appxinfo['_desa_db_host_'].";dbname=".$appxinfo['_desa_db_name_']."","".$appxinfo['_desa_db_user_']."","".$appxinfo['_desa_db_pass_']."");
				$dbcon->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				/* counting data */
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						/* $stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RT_WP_SPPT,RW_WP_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,PETUGASIDX,NAMAPETUGAS FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX > 0 ORDER BY KD_BLOK,NO_URUT,KD_JNS_OP"); */
						$stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RT_WP_SPPT,RW_WP_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,PETUGASIDX,NAMAPETUGAS FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak ORDER BY KD_BLOK,NO_URUT,KD_JNS_OP");
	...
	/*----------------------------------------------------------------
	 * RETRIEVE DATA SPPT/NOP UNTUK MATERI PEMBAYARAN PBB KE BANK
	 * ---------------------------------------------------------------*/
    } elseif ($_GET['cmdx'] == "GET_MATERI_SETOR_PEMBAYARAN_KE_BANK_PER_PETUGAS") {
					...
					if (intval($rowsfound)>0) {
						...
						if ($stmt_summary->execute()) {
							while($rowset_summary = $stmt_summary->fetch(PDO::FETCH_ASSOC)){
								$totalcountsppt = $rowset_summary['totalcountsppt']; $totalsumsppt = $rowset_summary['totalsumsppt']; $countallpenetapan = $rowset_summary['countallpenetapan'];$countdefaultpenetapan = $rowset_summary['countdefaultpenetapan']; $sumdefaultpenetapan = $rowset_summary['sumdefaultpenetapan'];$countlaterpenetapan = $rowset_summary['countlaterpenetapan']; $sumlaterpenetapan = $rowset_summary['sumlaterpenetapan'];$countunassigned = $rowset_summary['countunassigned']; $sumunassigned = $rowset_summary['sumunassigned'];$countassigned = $rowset_summary['countassigned']; $sumassigned = $rowset_summary['sumassigned'];$countunpaiddesa = $rowset_summary['countunpaiddesa']; $sumunpaiddesa = $rowset_summary['sumunpaiddesa'];$countpaiddesa = $rowset_summary['countpaiddesa']; $sumpaiddesa = $rowset_summary['sumpaiddesa'];$countunpaidunassigned = $rowset_summary['countunpaidunassigned']; $sumunpaidunassigned = $rowset_summary['sumunpaidunassigned'];$countpaidunassigned = $rowset_summary['countpaidunassigned']; $sumpaidunassigned = $rowset_summary['sumpaidunassigned'];$countunpaidassigned = $rowset_summary['countunpaidassigned'];$sumunpaidassigned = $rowset_summary['sumunpaidassigned'];$countpaidassigned = $rowset_summary['countpaidassigned'];$sumpaidassigned = $rowset_summary['sumpaidassigned'];$countunpaidassignedpetugas = $rowset_summary['countunpaidassignedpetugas'];$sumunpaidassignedpetugas = $rowset_summary['sumunpaidassignedpetugas'];$countpaidassignedpetugas = $rowset_summary['countpaidassignedpetugas'];$sumpaidassignedpetugas = $rowset_summary['sumpaidassignedpetugas'];$sumspptpetugas = $rowset_summary['sumassignedpetugas'];
							}
							$stmt_select_belum_bayar = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,TGL_TERBIT_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT,PETUGASIDX,NAMAPETUGAS FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX = :idxpetugas AND STATUS_PEMBAYARAN_SPPT = 0 AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01'");
							...
							$response = array(
								...
								"countspptpetugas"=>$rowsfound,
								"sumspptpetugas"=>$sumspptpetugas,
								"countunpaidassignedpetugas"=>$countunpaidassignedpetugas,
								"sumunpaidassignedpetugas"=>$sumunpaidassignedpetugas,
								"countpaidassignedpetugas"=>$countpaidassignedpetugas,
								"sumpaidassignedpetugas"=>$sumpaidassignedpetugas,
	...
	/*------------------------------------------------------------------------
	 * RETRIEVE DATA SPPT/NOP UNTUK MATERI DISTRIBUSI SPPT KOLEKTIF BY PETUGAS
	 * --------------------------------------------------------------------- */
    } elseif ($_GET['cmdx'] == "GET_MATERI_DISTRIBUSI_SPPT_KOLEKTIF_BY_PETUGAS") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						...
						if ($stmt_summary->execute()) {
							...
							$stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,TGL_TERBIT_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX = 0");
							...
							$response = array(
								"ajaxresult"=>"found",
								"datacontext"=>"GET_MATERI_DISTRIBUSI_SPPT_KOLEKTIF_BY_PETUGAS",
								"countpersonil"=>$countpersonil,
								"namakecamatan"=>$namaKecamatan,
								"namakelurahan"=>$namaKelurahan,
								"thnpajak"=>$thnsppt,
	...
	/*------------------------------------------------------------------------
	 * RETRIEVE DATA SPPT/NOP UNTUK MATERI DISTRIBUSI SPPT KOLEKTIF BY BLOK
	 * --------------------------------------------------------------------- */
    } elseif ($_GET['cmdx'] == "GET_MATERI_DISTRIBUSI_SPPT_KOLEKTIF_BY_BLOK") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
    	...
    	$rawpetugasidx = $_GET['idpetugas'];
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						...
						if ($stmt_summary->execute()) {
							...
							$stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,TGL_TERBIT_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND KD_BLOK = :kdblok AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX = 0");
							$stmt_select_data->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kdblok", $xblockidx, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":thnpajak", $thnsppt, PDO::PARAM_STR);
							$stmt_select_data->execute();
							while($rowset_select_data = $stmt_select_data->fetch(PDO::FETCH_ASSOC)){
								$items[] = $rowset_select_data;
							...
							$response = array(
								"ajaxresult"=>"found",
								"datacontext"=>"GET_MATERI_DISTRIBUSI_SPPT_KOLEKTIF_BY_BLOK",
								"blokidx"=>$xblockidx,
								"countpersonil"=>$countpersonil,
								"namakecamatan"=>$namaKecamatan,
								"namakelurahan"=>$namaKelurahan,
			...
			try {
				...
				/* ======= table: dhkp_temporary_sppt_data ======== */
				/* counting data */
				/* ---- disabled ---- $stmt_cnt = $dbcon->prepare("SELECT COUNT(*) as foundrows FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX > 0"); */
				$stmt_cnt = $dbcon->prepare("SELECT COUNT(*) as foundrows FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak");
				$stmt_cnt->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
				$stmt_cnt->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
				$stmt_cnt->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						...
						if ($stmt_summary->execute()) {
							...
							/* select all assigned */
							/* ---- disabled ---- $stmt_select_data = $dbcon->prepare("SELECT CONCAT(KD_PROPINSI,'.',KD_DATI2,'.',KD_KECAMATAN,'.',KD_KELURAHAN,'.',KD_BLOK,'-',NO_URUT,'-',KD_JNS_OP) AS NOP,THN_PAJAK_SPPT,NM_WP_SPPT,CONCAT(JLN_WP_SPPT,', ',BLOK_KAV_NO_WP_SPPT,', RT ',RT_WP_SPPT,'/ RW ',RW_WP_SPPT) AS ALAMAT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,TGL_TERBIT_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT,PETUGASIDX,NAMAPETUGAS FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX > 0 ORDER BY KD_BLOK,NO_URUT,KD_JNS_OP"); */
							$stmt_select_data = $dbcon->prepare("SELECT CONCAT(KD_PROPINSI,'.',KD_DATI2,'.',KD_KECAMATAN,'.',KD_KELURAHAN,'.',KD_BLOK,'-',NO_URUT,'-',KD_JNS_OP) AS NOP,THN_PAJAK_SPPT,NM_WP_SPPT,CONCAT(JLN_WP_SPPT,', ',BLOK_KAV_NO_WP_SPPT,', RT ',RT_WP_SPPT,'/ RW ',RW_WP_SPPT) AS ALAMAT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,TGL_TERBIT_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT,PETUGASIDX,NAMAPETUGAS FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak ORDER BY KD_BLOK,NO_URUT,KD_JNS_OP");
							$stmt_select_data->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
							$stmt_select_data->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR);
	 ...
	 * ---------------------------------------------------------------*/
			...
			try {
				...
				$stmt_create_dhkp_temporary_neu_sppt_data->execute();
				/* Insert Data to temporary SPPT */
				$stmt_insert_sppt_data = $dbcon->prepare("INSERT INTO dhkp_temporary_sppt_data(KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,KD_KANWIL_BANK,KD_KPPBB_BANK,KD_BANK_TUNGGAL,KD_BANK_PERSEPSI,KD_TP,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,KELURAHAN_WP_SPPT,KOTA_WP_SPPT,KD_POS_WP_SPPT,NPWP_SPPT,NO_PERSIL_SPPT,KD_KLS_TANAH,THN_AWAL_KLS_TANAH,KD_KLS_BNG,THN_AWAL_KLS_BNG,TGL_JATUH_TEMPO_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,NJOP_BUMI_SPPT,NJOP_BNG_SPPT,NJOP_SPPT,NJOPTKP_SPPT,NJKP_SPPT,PBB_TERHUTANG_SPPT,FAKTOR_PENGURANG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,STATUS_TAGIHAN_SPPT,STATUS_CETAK_SPPT,TGL_TERBIT_SPPT,TGL_CETAK_SPPT,NIP_PENCETAK_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT,TGL_REKAM_BYR_SPPT,NIP_REKAM_BYR_SPPT,PETUGASIDX,NAMAPETUGAS,STATUS) SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,KD_KANWIL_BANK,KD_KPPBB_BANK,KD_BANK_TUNGGAL,KD_BANK_PERSEPSI,KD_TP,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,KELURAHAN_WP_SPPT,KOTA_WP_SPPT,KD_POS_WP_SPPT,NPWP_SPPT,NO_PERSIL_SPPT,KD_KLS_TANAH,THN_AWAL_KLS_TANAH,KD_KLS_BNG,THN_AWAL_KLS_BNG,TGL_JATUH_TEMPO_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,NJOP_BUMI_SPPT,NJOP_BNG_SPPT,NJOP_SPPT,NJOPTKP_SPPT,NJKP_SPPT,PBB_TERHUTANG_SPPT,FAKTOR_PENGURANG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,STATUS_PEMBAYARAN_SPPT,STATUS_TAGIHAN_SPPT,STATUS_CETAK_SPPT,TGL_TERBIT_SPPT,TGL_CETAK_SPPT,NIP_PENCETAK_SPPT,PEMBAYARAN_SPPT_KE,DENDA_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT,TGL_REKAM_BYR_SPPT,NIP_REKAM_BYR_SPPT,PETUGASIDX,NAMAPETUGAS,STATUS FROM sppt_data USE INDEX(KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,TGL_TERBIT_SPPT) WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND TGL_TERBIT_SPPT = :defaulttglterbit ORDER BY NO_URUT, KD_JNS_OP");
				$stmt_insert_sppt_data->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
				$stmt_insert_sppt_data->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
				$stmt_insert_sppt_data->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
				$stmt_insert_sppt_data->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR);
				...
				if (intval($rowsfound)>0) {
					...
					/* BUILD SUMMARY */
					/* ======= table: sppt_data ======== */
					$stmt_summary_sppt_data = $dbcon->prepare("
						SELECT COUNT(*) AS totalcountsppt, 
							COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) AS totalsumsppt, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkeca AND KD_KELURAHAN = :kddesaa AND THN_PAJAK_SPPT = :thnpajaka) AS countallpenetapan, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_TERBIT_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecb AND KD_KELURAHAN = :kddesab AND THN_PAJAK_SPPT = :thnpajakb AND TGL_TERBIT_SPPT = CONCAT(:thnpajakbb,'-01-02')) AS countdefaultpenetapan, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_TERBIT_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecc AND KD_KELURAHAN = :kddesac AND THN_PAJAK_SPPT = :thnpajakc AND TGL_TERBIT_SPPT = CONCAT(:thnpajakcc,'-01-02')) AS sumdefaultpenetapan, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_TERBIT_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecd AND KD_KELURAHAN = :kddesad AND THN_PAJAK_SPPT = :thnpajakd AND TGL_TERBIT_SPPT != CONCAT(:thnpajakdd,'-01-02')) AS countlaterpenetapan, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_TERBIT_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkece AND KD_KELURAHAN = :kddesae AND THN_PAJAK_SPPT = :thnpajake AND TGL_TERBIT_SPPT != CONCAT(:thnpajakee,'-01-02')) AS sumlaterpenetapan, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecf AND KD_KELURAHAN = :kddesaf AND THN_PAJAK_SPPT = :thnpajakf AND PETUGASIDX = 0) AS countunassigned, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecg AND KD_KELURAHAN = :kddesag AND THN_PAJAK_SPPT = :thnpajakg AND PETUGASIDX = 0) AS sumunassigned, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX(KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkech AND KD_KELURAHAN = :kddesah AND THN_PAJAK_SPPT = :thnpajakh AND PETUGASIDX > 0) AS countassigned, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkeci AND KD_KELURAHAN = :kddesai AND THN_PAJAK_SPPT = :thnpajaki AND PETUGASIDX > 0) AS sumassigned, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecj AND KD_KELURAHAN = :kddesaj AND THN_PAJAK_SPPT = :thnpajakj AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01') AS countunpaiddesa, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkeck AND KD_KELURAHAN = :kddesak AND THN_PAJAK_SPPT = :thnpajakk AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01') AS sumunpaiddesa, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecl AND KD_KELURAHAN = :kddesal AND THN_PAJAK_SPPT = :thnpajakl AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01') AS countpaiddesa, 
							(SELECT COALESCE(SUM(JML_SPPT_YG_DIBAYAR), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecm AND KD_KELURAHAN = :kddesam AND THN_PAJAK_SPPT = :thnpajakm AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01') AS sumpaiddesa, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecn AND KD_KELURAHAN = :kddesan AND THN_PAJAK_SPPT = :thnpajakn AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01' AND PETUGASIDX = 0) AS countunpaidunassigned, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkeco AND KD_KELURAHAN = :kddesao AND THN_PAJAK_SPPT = :thnpajako AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01' AND PETUGASIDX = 0) AS sumunpaidunassigned, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecp AND KD_KELURAHAN = :kddesap AND THN_PAJAK_SPPT = :thnpajakp AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = 0) AS countpaidunassigned, 
							(SELECT COALESCE(SUM(JML_SPPT_YG_DIBAYAR), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecq AND KD_KELURAHAN = :kddesaq AND THN_PAJAK_SPPT = :thnpajakq AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX = 0) AS sumpaidunassigned, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecr AND KD_KELURAHAN = :kddesar AND THN_PAJAK_SPPT = :thnpajakr AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01' AND PETUGASIDX > 0) AS countunpaidassigned, 
							(SELECT COALESCE(SUM(PBB_YG_HARUS_DIBAYAR_SPPT), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecs AND KD_KELURAHAN = :kddesas AND THN_PAJAK_SPPT = :thnpajaks AND JML_SPPT_YG_DIBAYAR = 0 AND TGL_PEMBAYARAN_SPPT = '1900-01-01' AND PETUGASIDX > 0) AS sumunpaidassigned, 
							(SELECT COUNT(*) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkect AND KD_KELURAHAN = :kddesat AND THN_PAJAK_SPPT = :thnpajakt AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX > 0) AS countpaidassigned, 
							(SELECT COALESCE(SUM(JML_SPPT_YG_DIBAYAR), 0) FROM sppt_data FORCE INDEX (KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT,TGL_PEMBAYARAN_SPPT) WHERE KD_PROPINSI = '33' AND KD_DATI2 = '23' AND KD_KECAMATAN = :kdkecu AND KD_KELURAHAN = :kddesau AND THN_PAJAK_SPPT = :thnpajaku AND JML_SPPT_YG_DIBAYAR > 0 AND TGL_PEMBAYARAN_SPPT != '1900-01-01' AND PETUGASIDX > 0) AS sumpaidassigned 
						FROM sppt_data 
							FORCE INDEX (KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,THN_PAJAK_SPPT) 
						WHERE 
							KD_PROPINSI = '33' 
							AND KD_DATI2 = '23' 
					...
					$stmt_summary_sppt_data->bindValue(":kdkeca", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesaa", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajaka", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecb", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesab", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakb", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakbb", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecc", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesac", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakc", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakcc", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecd", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesad", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakd", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakdd", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkece", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesae", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajake", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakee", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecf", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesaf", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakf", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecg", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesag", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakg", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkech", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesah", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakh", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkeci", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesai", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajaki", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecj", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesaj", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakj", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkeck", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesak", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakk", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecl", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesal", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakl", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecm", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesam", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakm", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecn", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesan", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakn", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkeco", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesao", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajako", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecp", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesap", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakp", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecq", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesaq", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakq", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecr", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesar", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakr", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecs", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesas", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajaks", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkect", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesat", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajakt", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkecu", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesau", $kode_kelurahan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":thnpajaku", $thnsppt, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR); 
					$stmt_summary_sppt_data->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR); 
					...
					$stmt_summary_dhkp_local_b = $dbcon->prepare("
						SELECT 
							...
							COALESCE(SUM(JML_SPPT_YG_DIBAYAR), 0) AS sumpaiddefault 
						FROM 
							dhkp_temporary_sppt_data USE INDEX(TGL_TERBIT_SPPT) 
						WHERE 
							TGL_TERBIT_SPPT = :defaulttglterbit");
					$stmt_summary_dhkp_local_b->bindValue(":defaulttglterbit", $tglterbitdefault, PDO::PARAM_STR);
					$stmt_summary_dhkp_local_b->execute();
	...
	/*----------------------------------------------------------------
	 * RETRIEVE DATA DAFTAR HIMPUNAN KETETAPAN PAJAK (DHKP) DARI MYSQL BY TAX YEAR
	 * ---------------------------------------------------------------*/
    } elseif ($_GET['cmdx'] == "GET_MYSQL_DHKP_LOCAL_BYTAXYEAR") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
		...
		} elseif ($_GET['cmdx'] == "GET_MYSQL_PEMBAYARAN_LOCAL_BYTAXYEAR") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
			...
			try {
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						...
						if ($stmt_summary_all_data->execute()) {
							...
							if ($stmt_select_data->execute()) {
								...
								$response = array("ajaxresult"=>"notfound","message"=>"");
								header('Content-Type: application/json');
								echo json_encode($response);
								$dbcon = null;
								exit;
							...
							$response = array("ajaxresult"=>"notfound","message"=>"XX");
							header('Content-Type: application/json');
							echo json_encode($response);
							$dbcon = null;
							exit;
						...
						$response = array("ajaxresult"=>"notfound","message"=>"XXX");
						header('Content-Type: application/json');
						echo json_encode($response);
						$dbcon = null;
						exit;
					...
					$response = array("ajaxresult"=>"notfound","message"=>"XXXX");
					header('Content-Type: application/json');
					echo json_encode($response);
					$dbcon = null;
					exit;
		...
		} elseif ($_GET['cmdx'] == "GET_MYSQL_PENAGIHAN_LOCAL_BYTAXYEAR") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
		...
		} elseif ($_GET['cmdx'] == "PREVIEW_ASSIGNED_SPPT_PETUGAS") {
			$kode_propinsi = $_SESSION['desaXweb']['KD_PROPINSI'];
			$kode_dati2 = $_SESSION['desaXweb']['KD_DATI2'];
			$kode_kecamatan = $_SESSION['desaXweb']['KD_KECAMATAN'];
			$kode_kelurahan = $_SESSION['desaXweb']['KD_KELURAHAN'];
			...
			try {
				...
				$dbcon->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				/* counting data */
				$stmt_select_data = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,TGL_TERBIT_SPPT,STATUS_PEMBAYARAN_SPPT,JML_SPPT_YG_DIBAYAR,TGL_PEMBAYARAN_SPPT FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND THN_PAJAK_SPPT = :thnpajak AND PETUGASIDX = :idxpetugas ORDER BY KD_BLOK,NO_URUT,KD_JNS_OP");
				$stmt_select_data->bindValue(":kdprov", $kode_propinsi, PDO::PARAM_STR);
				$stmt_select_data->bindValue(":kdkab", $kode_dati2, PDO::PARAM_STR);
				$stmt_select_data->bindValue(":kdkec", $kode_kecamatan, PDO::PARAM_STR);
				$stmt_select_data->bindValue(":kddesa", $kode_kelurahan, PDO::PARAM_STR);
				$stmt_select_data->bindValue(":thnpajak", $thnsppt, PDO::PARAM_STR);
				$stmt_select_data->bindValue(":idxpetugas", $petugasidx, PDO::PARAM_INT);
				if ($stmt_select_data->execute()) {
					while($rowset_select_data = $stmt_select_data->fetch(PDO::FETCH_ASSOC)){
						$items[] = $rowset_select_data;
					}
			...
			try {
				...
				if ($stmt_cnt->execute()) {
					...
					if (intval($rowsfound)>0) {
						$stmt = $dbcon->prepare("SELECT KD_PROPINSI,KD_DATI2,KD_KECAMATAN,KD_KELURAHAN,KD_BLOK,NO_URUT,KD_JNS_OP,THN_PAJAK_SPPT,NM_WP_SPPT,JLN_WP_SPPT,BLOK_KAV_NO_WP_SPPT,RW_WP_SPPT,RT_WP_SPPT,LUAS_BUMI_SPPT,LUAS_BNG_SPPT,PBB_YG_HARUS_DIBAYAR_SPPT,TGL_TERBIT_SPPT FROM sppt_data WHERE KD_PROPINSI = :kdprov AND KD_DATI2 = :kdkab AND KD_KECAMATAN = :kdkec AND KD_KELURAHAN = :kddesa AND KD_BLOK = :kdblok AND THN_PAJAK_SPPT = :thnpajak ORDER BY NO_URUT");
